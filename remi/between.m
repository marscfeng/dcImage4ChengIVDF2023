...
%    if flag == -2, then outside x1-x2 without x1/x2 included
%  ******** flag defaults to 0 ****
%
...
%
% History:
% 	make indicies as empty when no index detected, F. Cheng, 01-Apr-2020
%

if nargin < 4, flag =2; end
...
if flag == -2
    if( x1 < x2)
        indicies = union(find((testpts < x1)), find((testpts > x2)));
        % if(isempty(indicies))indicies=0;end
        return;
    else
        indicies = union(find((testpts < x2)), find((testpts > x1)));
        % if(isempty(indicies))indicies=0;end
        return;
    end
end
